...
| **quality (perplexity)** | **not comparably measurable yet**, with a reason: our stepwise scoring does not reproduce a batched prefill on this model (cosine 0.970, and the two paths pick different next tokens), while `llama-perplexity` evaluates a chunk as a batch. A 78% gap measured that way is withdrawn. Blocked on `score-a-chunk-from-one-batched-pass.md` | **25.9308 vs 25.8347 ± 2.78 — +0.37%**, agreement | **44.8667 vs 45.5245 ± 5.70 — −1.44%**, Chaos slightly *better*, on real prose at chunk 512 with per-chunk BOS. The old **33.6434 vs 34.0293** is stale: it predates the BOS fix |
