---
topic: STATUS.md's history — every dated section it accumulated, 2026-07-28 to 2026-08-28
status: archive
links:
  - ../../../STATUS.md
  - ../INDEX.md
...
**This is where `STATUS.md` kept its history until 2026-08-31, and why it had to
stop.** The file called itself *"the single place that says what is true today"*
and had grown to **5,144 lines across 104 dated sections** — a reverse-chronological
diary in which the current truth was the first 60 lines and everything after it was
the past, unmarked as past.

That is not a filing problem, it is a correctness problem. An audit on 2026-08-31
found **22 contradictions** between this file and the three other scoreboards, and
several were this file disagreeing with *itself*: it recorded both sides of three
reversed decisions — the `/status` gate, the llama.cpp pin, and whether `chaos-qr`
had shipped — without marking either side stale. A reader had no way to tell which
paragraph was current, and neither did a writer.

**Nothing here is deleted and nothing is edited.** Every section below is verbatim
as it stood, in the order it was written, newest first. It is kept because the
reasoning is worth more than the conclusions: most of these sections are a
measurement that killed an idea, and re-killing an idea is expensive.

**Do not cite this file for anything current.** `STATUS.md` is the scoreboard;
`CHECKLIST.md` is the tick-list; the graph nodes under `docs/graph/` hold the
reasoning. If one of those disagrees with a line here, the line here is history.
...
**Current**: **959 tests** (0 failed, 42 ignored — the V4-Flash set
