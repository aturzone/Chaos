...
| L4 | **v0.0.27 — Quality harness**, then the levers | **[~]** the harness is in (C6) and **two levers are through it**: **C5e** (1.120x, exact, 50/50) and **the expert cache default** (1.20x, exact, 50/50) — **0.509 → 0.728 tok/s together**. Left: **C7**, whose case is now narrower — the dtype argument is dead (Q8_0 is the *fastest* of three at the trunk's shape), so only the cache-cliff argument survives |
...
| L7 | **v0.0.30 — LTS**: the parity gate, the competitive claim, a support policy | **[~]** **2 of 18 cells met, up from 1**, and three more measured rather than untested — but the night's parity work mostly found problems: V4-Flash generation is **`unmeasurable here`** (llama.cpp 0.16–0.47 for one command line), long-context generation on Qwen3-4B is **1.45x behind**, and the dense ratio **flips** with context (1.09x ahead at 500 tokens, 1.38x behind at 4031, Chaos's slope 2.2x steeper). Knowing that is progress; meeting the bar is not yet |
