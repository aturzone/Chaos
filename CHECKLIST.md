...
| C5d | Convert `ffn_gate_inp` from BF16 at load | **[ ]** the router's whole cost is a **4096x256 BF16 matmul at 6.4 ms** — 1.05M MACs, two orders of magnitude slower than the shape deserves. ggml has no x86 fast path for Q8_0 either, so a missing BF16 path is the suspect. **Untested**: measure BF16 vs F16 vs F32 at that shape in `chaos-kernelbench` first. Costs ~90 MB of residency against a 7.38 GiB trunk, and it is **exact** — BF16 to F32 is the same value — so it goes behind the *exact* bar. Ceiling **1.13x**, and only if the matmul becomes free |
