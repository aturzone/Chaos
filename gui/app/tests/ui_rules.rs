...
///
/// **There is one mode now, so there is nothing to gate.** This used to open on
/// "the first page the mode can reach", because a HELPER had no CHAT and
/// starting it there would have raised a page the rail could not reach. With
/// the knob gone and every page in the rail, the answer is simply the first
/// rail entry.
#[test]
fn the_window_opens_on_chat() {
    let src = main_rs();
    assert!(
        code_only(&src).contains("let startup_page = nav::RAIL_PAGES[0]"),
        "the window no longer opens on the rail's first page"
    );
    assert_eq!(nav::RAIL_PAGES[0], Page::Chat);
    assert_eq!(nav::PAGES[0], Page::Chat);
}

/// **The launch screen is gone, and nothing may bring it back.**
///
/// The knob owned the whole window until a mode was chosen: it painted over
/// every child control, `launched` gated `WM_PAINT`, and Escape dropped a
/// loaded model in one keystroke. Atur, 2026-09-07: *"that mode selection
/// isn't needed any more, because we have one mode that includes everything"*.
///
/// A source-level check because the failure it prevents is a re-introduction,
/// not a runtime state: any of these names coming back means the window can
/// again open onto something that is not a page.
#[test]
fn there_is_no_launch_screen() {
    ...
    for gone in [
        ...
        assert!(
            !src.contains(gone),
            "{gone} is back, so the window may open on something that is not a page"
        );
    }
}

/// **The role is still remembered, and is now a setting like any other.**
///
/// It was asked by a launch knob and carried by a separate `mode_chosen` flag,
/// because `role`'s own default is a real role and could not say "unanswered".
/// With no launch screen there is no question to remember the answer to: the
/// role is read from the file, shown in a dropdown on the CHAOS page, and
/// written back when it changes.
#[test]
fn the_role_is_read_and_written_like_any_setting() {
    let src = code_only(&main_rs());
    assert!(
        src.contains("fn pick_role(role: settings::Role)"),
        "the role is no longer set from a value; it was taking a control id"
    );
    assert!(
        src.contains("sync_role_combo"),
        "nothing puts the stored role back into the dropdown"
    );
    // The dropdown's order lives in exactly one place, so an index cannot
    // start meaning a different role than the list shows.
    assert_eq!(
        src.matches("const ROLES:").count(),
        1,
        "the role order is written more than once"
    );
}
