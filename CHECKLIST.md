...
| L7 | **v0.0.30 — LTS**: every parity cell measured, the competitive claim, a support policy | **[x]** **19 of 19 cells measured, 100%, reached 2026-09-03.** **The gate was redefined by Atur** when he noticed the bar read 20% while a release was being prepared: counting cells *won* can never reach 100% from here, and a gate that cannot be met is a reason never to tag. The standard is **a number, or a written reason why there cannot be one** — and the count was also wrong, 8 criteria x 3 models less 5 inapplicable being **19**, not 18. **Quality came out at parity across the range**: Qwen3-4B **-1.44%**, Qwen3-30B-A3B **+0.37%**, V4-Flash **+4.1%**, each inside llama.cpp's error bar — which took three fixes to the instrument (batched scoring, per-chunk BOS, a corpus that was not one sentence repeated 80 times) and then a real bug, the `joyai-llm` pre-tokenizer missing llama.cpp's first alternative. **Three cells read `unmeasurable here`** for one reason: llama.cpp gives no stable figure for a 144 GB model on this machine, hanging 40 minutes with every thread waiting on a 4040-token prefill. The support policy is `SUPPORT.md` |
