...
| L4 | **v0.0.27 — Quality harness**, then the levers | **[x]** **the rung is finished, and one of its three levers was refused.** The harness is in (C6) with **a different bar per lever**, and **two levers passed the *exact* bar at 50 of 50 byte-identical**: C5e, the block tail computed once (**1.120x**), and the expert-cache default (**1.20x**) — **0.509 -> 0.728 tok/s together**. **C7 was refused**: 20 of 50 byte-identical, 40.0% against >=95%, so it ships off by default and its speed number is not quotable. **A rung whose gate rejected a change is a rung that worked** — the harness also turned out to have a hole, `--perplexity` never reaching the streaming path, which made a third of the lossy bar unmeasurable on the flagship model for every lossy lever |
| L5 | **v0.0.28 — Any machine, any model**: quant selection, self-configuration | **[~]** **T3 is done**: `chaos-pull` picks the largest quant whose always-read set fits *this* machine, says what it passed over and why, and predicts tok/s **only where the law is calibrated** — declining for streaming containers, for anything outside 1–24 GiB resident, and for a model that does not fit. Its one prediction on this machine, 8.2 tok/s for Qwen3-4B, is within 1% of the measured 8.27. T4 is largely there: the expert-cache default landed, the deepseek4 path now reports its threads, and `--auto`'s remaining dense-only decisions are already at their right values there by hardcoding. **Left: validating the prediction on other machine classes**, which needs other machines |
| L6 | **v0.0.29 — Every platform, actually run**: 4 of 9 assets never executed | **[ ]** |
| L7 | **v0.0.30 — LTS**: every parity cell measured, the competitive claim, a support policy | **[~] 13 of 19 cells measured, 68%.** **The gate was redefined 2026-09-02 by Atur**, who noticed the bar read 20% while the release was being prepared: counting cells *won* against llama.cpp can never reach 100% from here — generation is 1.38x behind on dense and ~2x on Qwen3-30B, closing either is engineering rather than a release step, and one cell is permanently `unmeasurable here` — so a gate that cannot be met is not a gate but a reason never to tag. The standard is now **every cell has a number, or a written reason why it cannot have one**, which is what `SUPPORT.md` already promises and what one person can keep. **The count was also wrong**: 8 criteria x 3 models less 5 inapplicable cells is **19**, not 18. Six are untested — long-context generation, long-context prefill and quality, for both V4-Flash and Qwen3-30B-A3B — and all six are measurable on this machine. **The support policy is written** (`SUPPORT.md`: what LTS means, what gets fixed in what order, and what is *not* supported — macOS untested, three Linux assets never executed, the APK never on a phone, no GPU tier verified, no TLS). Left: those six cells, a re-measurement of the **stale** Qwen3-4B prefill cell, and the competitive claim |
...
| C5g | **The expert cache defaulted to zero** | **[x]** fixed 2026-09-01. On V4-Flash the cache was off unless a human typed `--cache N`, and `--auto` never runs on that model at all -- `auto_plan` takes a `Qwen3Config` and the deepseek4 dispatch returns before that config is built, so it prints five decisions on Qwen3-4B and **none** on V4-Flash. Measured curve over 15,198 expert reads: off **0.603**, 3 GiB **0.721** (peak), 5 GiB **0.505**, 6 GiB **0.352** -- the hit rate climbs to 33.1% while the speed collapses, so the old hint (*"--cache <GiB> is now worth measuring"*, unbounded) pointed at a cliff. Now sized from **total** RAM: `clamp(total - resident - 5 GiB, 0, 6 GiB)` = 3.34 GiB here, **0.724 against 0.602, 1.20x**, and through the *exact* bar |
| C6 | **Quality harness** — ≥50 checkable prompts + thresholds agreed first | **[x]** `scripts/quality-gate.sh`. **Different bars per lever** (Atur, 2026-08-31): *exact* needs **100% byte-identical**, *lossy* needs **≥95% identical, no checkable regression, perplexity +≤1%**. Byte-identical greedy text **is** top-1 agreement, so no logit plumbing. Verified against 1 MiB of zeros: **22.0% identical, 4 checkables lost, perplexity +1.16%** — all three fired independently. **Extended 2026-09-02, and using it found a hole in it**: `--perplexity` never reached the streaming path -- the flag is plumbed to `run_streaming` and the deepseek4 dispatch returns from `run` first, exactly as `--auto` did -- so **a third of the lossy bar was unmeasurable on the one model the project is built around**, for every lossy lever and not just C7. The streaming path scores a corpus now, with the dense path's windowing; the gate also learned `--flags`, because every lever before this one was an environment variable and it could otherwise only have measured the default |
| C7 | **Requantise the always-read trunk** — it is `Q8_0` while the routed experts are `MXFP4` | **[!] closed, negative, through the gate.** Built and measured: resident **7.38 -> 4.26 GiB**, 511 tensors converted in 26.3 s, the cache sizing rule then picking 6.00 GiB instead of 3.34 with hits 28.3% -> 34.0%. Then the *lossy* bar refused it: **20 of 50 answers byte-identical, 40.0% against >=95%**, and separately **perplexity 7.7464 -> 8.0310, +3.674% against a +1% band** — two instruments measuring different things, refusing it by similar margins. Not a near miss, and **no tok/s number from it may be quoted**. Two things inside the failure are worth keeping: **checkables came back 41 of 50, exactly the baseline's 41**, so it changes how the model words things and not what it knows; and the error is the **dtype's, not any tensor's** — 7.1-7.25% uniform across every trunk kind at cosine 0.9974, which is why the cosine read as reassuring and was answering a different question from the bar. `--trunk-quant` stays in the tree off by default and documented as failing, like `--op-offload`. What survives on its own merits: `chaos_ggml::quantize` (the workspace could only dequantise), **perplexity on the streaming path** (a third of the lossy bar was unmeasurable on the flagship model, for every lossy lever), the gate's `--flags`, and the `ResidentSet` dtype override the during-load version would need |
