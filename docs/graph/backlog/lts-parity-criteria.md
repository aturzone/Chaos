...
| long-context generation | **0.589 tok/s at 4040 tokens** — it ran for the first time on 2026-09-02, after `-b` was plumbed to this path and the arena stopped being a hardcoded 1 GiB; llama.cpp not yet run at this length, and the engine warns its own logits are APPROXIMATE above ~2051 tokens | **3.06 vs 2.22 — 1.38x AHEAD**, and Chaos's worst run (2.77) beats llama.cpp's best (2.50), which fell 2.50 → 2.22 → 1.63 across three runs while Chaos held | **3.13 vs 4.53 — 1.45x behind** at 4031 tokens, both engines stable. **The ratio FLIPS**: short context, same session, Chaos **8.39 vs 7.69 — 1.09x ahead**. Chaos slows **2.68x** with 4000 tokens of context against llama.cpp's **1.70x**. `../research/long-context-parity-qwen3-4b-2026-09-01.md` |
| long-context prefill | **5.07 tok/s at 4040 tokens**, 8 blocks of 512 — first run ever; llama.cpp not yet run at this length | **35.61 vs 39.54 — 1.11x behind**, ranges overlap (llama.cpp's worst, 29.73, is slower than all three Chaos runs) | **60.98 vs 59.77 — parity** (inside the spread) |
...
| **quality (perplexity)** | **14.6877 vs 14.1034 ± 1.79 — +4.1%**, inside llama.cpp's error bar. Took three fixes to measure at all: batched scoring (a stepwise pass does not reproduce a batched one here — cosine 0.970 and a different next token), per-chunk BOS, and a real corpus. Then it took a **real bug**: the `joyai-llm` pre-tokenizer was missing llama.cpp's first alternative, so `SUPPORT.md` split as `.` + `md`. **18.5548 → 14.6877** on that fix, ids now matching `llama-tokenize` 294 for 294 | **25.9308 vs 25.8347 ± 2.78 — +0.37%**, agreement | **44.8667 vs 45.5245 ± 5.70 — −1.44%**, Chaos slightly *better*, on real prose at chunk 512 with per-chunk BOS. The old **33.6434 vs 34.0293** is stale: it predates the BOS fix |
...
**19 of 19 measured — 100%, reached 2026-09-03.** Every cell now carries a number or a written reason. Three read `unmeasurable here`, all for the same competitor behaviour on a 144 GB model on this machine: llama.cpp returned 0.16–0.47 tok/s across eight runs of one command line, and on a 4040-token prefill it **hung for 40 minutes** with all eleven threads in `Wait`, zero CPU and four page faults in eight seconds. Our own numbers for those cells exist and are stated; what does not exist is a stable competitor figure to divide by.
